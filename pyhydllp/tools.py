...
from pyhydllp.util import select_sites, rd_dir


def rd_hydstra_bulk(varto, sites=None, data_source='A', from_date=None, to_date=None, from_mod_date=None, to_mod_date=None, interval='day', qual_codes=[30, 20, 10, 11, 21, 18], concat_data=False, export=None, code_convert=None, qual_code_convert=None):
    """
...
#    sites_list = sites_var_period.site.unique().tolist()
    ...
    for tup in sites_var_period2.itertuples(index=False):
        ...
        df = rd_hydstra_ts([tup.site], data_type=data_type, start=tup.from_date, end=tup.to_date, varfrom=tup.varfrom, varto=varto, interval=interval, qual_codes=qual_codes)
        if df.empty:
            continue
